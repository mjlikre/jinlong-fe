...
export const getClients = ({ callback }) => (dispatch, getState) => {
  try {
...
export const getClient = ({ clientId, callback }) => (dispatch, getState) => {
    try {
      const res = await axios.get(
...
export const deleteClient = ({ clientId, index, callback }) => (dispatch, getState) => {
  try {
    await axios.delete(
        `http://localhost:3001/client/${clientId}`, 
        { headers: { authorization: localStorage.getItem("token") } },
...
export const updateClient = ({ clientId, update, index, callback }) => (dispatch, getState) => {
  try {
...
export const createClient = ({ client, callback }) => (dispatch, getState) => {
  try {
